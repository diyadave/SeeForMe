...
class NameExtractor:
    ...
    def is_name_update(self, text: str, current_name: str = None) -> bool:
        ...
        update_patterns = [
            ...
            r"(?:correct|change)\s+(?:my name|name)\s+(?:to|is)\s+([a-zA-Z][a-zA-Z\s]{1,20})"
        ]
    ...
    def get_current_name(self) -> Optional[str]:
        """Get the most recently extracted name"""
        return self.known_names[-1] if self.known_names else None
    
    def clear_names(self):
        """Clear all stored names"""
        self.known_names.clear()
        logger.info("🗑️ Cleared all stored names")
    
    def get_status(self) -> Dict:
        """Get current status of name extractor"""
        return {
            'known_names_count': len(self.known_names),
            'current_name': self.get_current_name(),
            'recent_names': self.known_names[-5:] if self.known_names else []
        }
...
if __name__ == "__main__":
    ...
    # Test cases
    test_phrases = [
        ("Hello, my name is John Smith", "en"),
        ("Hi there, I'm Sarah", "en"),
        ("Namaste, main Raj hun", "hi"),
        ("Hu David chhu", "gu"),
        ("Call me Mike", "en"),
        ("Very good morning", "en"),  # Should not extract name
    ]
    
    for phrase, lang in test_phrases:
        name = extractor.extract_name(phrase, lang)
        print(f"'{phrase}' [{lang}] -> {name}")
